...
class Detector:
    ...
    def load(self,cam_para):
        self.mapper = Mapper(cam_para,"MOT17")
    ...
    def box_to_det(self, box, dets):
        det_id = len(dets)
        det = Detection(det_id)
        det.bb_left = box[0]
        det.bb_top = box[1]
        det.bb_width = box[2] - box[0]
        det.bb_height = box[3] - box[1]
        det.conf = 1
        det.det_class = 2 # car
        det.y,det.R = self.mapper.mapto([det.bb_left,det.bb_top,det.bb_width,det.bb_height])
        return det
