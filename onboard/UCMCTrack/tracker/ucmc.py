...
class UCMCTrack(object):
    ...
    def update(self, dets,frame_id, affine=None):

        self.data_association(dets,frame_id,affine)
    ...
    def data_association(self, dets,frame_id, affine=None):
        ...
        for track in self.trackers:
            ...
            if self.use_cmc:
                if affine is None:
                    raise ValueError("Affine matrix must be provided for CMC tracking.")
                x,y = self.detector.cmc(track.kf.x[0,0],track.kf.x[2,0],track.w,track.h,affine)
                track.kf.x[0,0] = x
                track.kf.x[2,0] = y
    ...
    def associate_tentative(self, dets):
        ...
        for i,j in matched_indices:
            ...
            self.trackers[trk_idx].detidx = det_idx
            dets[det_idx].track_id = self.trackers[trk_idx].id
            if self.trackers[trk_idx].birth_count >= 1:
