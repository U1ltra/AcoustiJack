...
int main()
{
    ...
    while (true) {
        ...
        // added code just to compile, not used...
        trax::ImageList imgList;
        imgList.set(img, 0);
        int tr = handle.wait(imgList, reg, prop);

        // int tr = handle.wait(img, reg, prop); // original code, compile error
